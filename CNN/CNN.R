...
#   image_array <-rbind(image_array,as.array(image))
# }
...
dim(image_array) <- c(length(data$url),300,300,3)
dim(image_array)
...
train_image <- array_reshape(train_image, c(nrow(train_image), 300, 300, 3))
test_image <- array_reshape(test_image, c(nrow(test_image), 300, 300, 3))
...
predict_classes(model,test_image)
